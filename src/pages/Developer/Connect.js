...
import { FaLinkedin, FaGithub, FaGlobe, FaUserCircle, FaTimes, FaCheck } from "react-icons/fa";  // Social icons
...
const Connect = () => {
  ...
  const [swipeDirection, setSwipeDirection] = useState(null); // Tracks current swipe direction
  const [isAnimating, setIsAnimating] = useState(false); // Prevents multiple swipes during animation
  ...
  useEffect(() => {
    const fetchDevelopers = async () => {
      try {
        const response = await axios.get("https://thrive-xbzt.onrender.com/api/developer/connect", {
          headers: { 
  ...
  const handleSwipe = useCallback(async (direction) => {
    if (developers.length === 0 || isAnimating) return;
    ...
    setTimeout(async () => {
      try {
        const response = await axios.post("https://thrive-xbzt.onrender.com/api/developer/connect", {
  ...
  }, [developers, currentIndex, loggedInDeveloperId, isAnimating]);
    ...
    <div>
      ...
      <div className="connect-container">
        ...
        {developers.length > 0 && currentIndex < developers.length ? (
          <div>
            <div className={swipeDirection ? `swipe-card swiping-${swipeDirection}` : "swipe-card"}>
              {/* 🔹 Left Column: Profile + Professional Details + Education */}
              <div className="left-column">
                {/* Profile Header */}
                {developers[currentIndex]?.profilePhoto ? (
                  <img 
                    src={`https://thrive-xbzt.onrender.com${developers[currentIndex].profilePhoto}`} 
                    alt="Profile" 
            ...
            <div className="swipe-buttons-container">
              <button 
                className="swipe-btn-connect swipe-left" 
              ...
              <button 
                className="swipe-btn-connect swipe-right" 
