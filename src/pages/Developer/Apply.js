...
const Apply = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      const developerId = localStorage.getItem("developerId");
      try {
        const response = await axios.get("https://thrive-xbzt.onrender.com/api/developer/jobs", {
  ...
  const handleSwipe = useCallback(async (action) => {
    ...
    try {
      const response = await axios.post("https://thrive-xbzt.onrender.com/api/developer/jobs", {
        jobId,
        action,
      }, {
  ...
  const handleBookmark = async () => {
    ...
    try {
      await axios.post("https://thrive-xbzt.onrender.com/api/developer/jobs", {
        jobId,
        action: "underHold",
      }, {
        headers: { "developer-id": developerId, "Content-Type": "application/json" },
      ...
      <div className="apply-container">
        {jobs.length > 0 && currentIndex < jobs.length ? (
          <div className="job-card">
            ...
            <p className="company-name">{jobs[currentIndex].companyName}</p>
            ...
            <div className="swipe-instructions">
              <button className="swipe-btn swipe-left" onClick={() => handleSwipe("swipeLeft")}>
                <FaTimesCircle /> Reject
              </button>
              <button className="swipe-btn bookmark-icon" onClick={handleBookmark}>
                <FaBookmark /> Save
              </button>
              <button className="swipe-btn swipe-right" onClick={() => handleSwipe("swipeRight")}>
                <FaCheckCircle /> Apply
              </button>
            </div>
          ...
          <p className="no-jobs-message">No more jobs to show.</p>
        )}
      </div>
    </div>
  );
